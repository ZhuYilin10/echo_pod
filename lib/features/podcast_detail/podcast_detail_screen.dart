...
class _PodcastDetailScreenState extends ConsumerState<PodcastDetailScreen> {
  ...
  Widget build(BuildContext context) {
    final isSubscribedAsync = ref.watch(isSubscribedProvider(widget.podcast.feedUrl));
          ...
          slivers: [
            SliverAppBar(
              ...
              flexibleSpace: FlexibleSpaceBar(
                title: Text(widget.podcast.title, style: const TextStyle(shadows: [Shadow(blurRadius: 10, color: Colors.black)])),
                background: Stack(
                  fit: StackFit.expand,
                  children: [
                    Image.network(widget.podcast.imageUrl ?? '', fit: BoxFit.cover),
                    Container(
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(widget.podcast.artist ?? '', style: Theme.of(context).textTheme.titleMedium),
                              const SizedBox(height: 4),
                              Text('${_allEpisodes.length} 剧集', style: Theme.of(context).textTheme.bodySmall),
                            ],
                          ),
                        ),
                        isSubscribedAsync.when(
                          data: (isSubscribed) => ElevatedButton(
                            onPressed: () async {
                              ...
                              if (isSubscribed) {
                                await storage.unsubscribe(widget.podcast.feedUrl);
                              } else {
